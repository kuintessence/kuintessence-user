<template>
  <q-page class="page-pt page-px q-pb-md">
    <div class="full-width frosted-glass-card official-template-div">
      <q-item class="q-pa-lg" style="background: rgba(255, 255, 255, 0.5); border-radius: 16px 16px 0 0">
        ...
        <q-item-section class="q-pl-md">
          <q-item-label lines="1" class="text-h5 text-weight-medium" style="line-height: 30px">
            <span class="text-grey-7">模板 > </span>
            <span class="q-pr-sm">{{ data.name ? data.name : '未命名模板' }}</span>
            <q-chip
              square
              :ripple="false"
              size="sm"
          ...
          <q-item-label class="q-pt-md">
            <q-btn unelevated color="primary" label="使用" @click="useFlow" class="q-mr-sm" />
            <q-btn
              unelevated
              color="white"
              text-color="black"
 ...
 * 使用模板
 */
async function useFlow(event: any) {
  $q.loading.show();
  try {
    let draftId = await DraftService.saveMyDraft({
      templateId: data.value.uuid,
      name: data.value.display_name
        ? data.value.display_name
        : data.value.system_name
        ? data.value.system_name
...
const loadData = async () => {
  ...
  if (route.query.id) {
    ...
    try {
      data.value = await ProjectService.getTemplateDetail(id.value);
      judgeIsFavorite();
      keepAliveStore.tabList.forEach(tab => {
        if (tab.fullPath === route.fullPath)
          tab.meta.title = data.value.name ? data.value.name + ' - 模板详情' : '未命名模板 - 模板详情';
      });
      loading.value = false;
    } catch (e) {
      loading.value = false;
