<template>
  <q-page class="page-pt page-px q-pb-md">
    <div class="full-width frosted-glass-card usecase-detail-div">
      ...
      <div class="row">
        ...
        <div class="col-auto">
          ...
          <div
            class="translucent-card q-mt-md q-mr-md q-pa-md"
            style="width: 300px"
          ...
          <div
            ...
            style="width: 300px"
            v-if="data.entity_dependency_obj && dependentVersions?.length > 0"
          >
            <div class="text-weight-medium text-grey-7">关联软件</div>
            <q-list class="q-mt-sm">
              <div v-for="(item, index) in dependentVersions" :key="item.contentDependencyId" @click="toSoftware">
...
import { NodeOriginData } from 'src/components/flow-draft-editor/interfaces';
...
const nodeOriginData = ref<any>(null);

const toSoftware = (event: any) => {
  const { x, y } = event;
  commonStore.animate(x, y, '/software-detail?id=' + data.value.entity_dependency_obj.uuid);
};
/**
 * 判断是否已收藏
 */
...
const getRelatedSoftware = async () => {
  if (data.value.content_entity_versions?.length > 0 && data.value.entity_dependency_obj)
