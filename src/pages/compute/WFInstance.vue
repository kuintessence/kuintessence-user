...
async function getInstanceLength() {
  instanceLength.value = await InstanceService.getListNumberWithType(undefined, 0);
  runningInstanceLength.value = await InstanceService.getRunningNumberWithType(undefined, 0);
  finishedInstanceLength.value = await InstanceService.getFinishedNumberWithType(undefined, 0);
  warningInstanceLength.value = await InstanceService.getWarningNumberWithType(undefined, 0);
}
const loadData = () => {
  resetSelect();
  onRequest({
...
const onRequest = async (props: any) => {
  ...
  try {
    ...
    pagination.value.rowsNumber =
      tab.value === '正在运行'
        ? await InstanceService.getRunningNumberWithType(filter, 0)
        : tab.value === '最新完成'
        ? await InstanceService.getFinishedNumberWithType(filter, 0)
        : tab.value === '需要注意'
        ? await InstanceService.getWarningNumberWithType(filter, 0)
        : await InstanceService.getListNumberWithType(filter, 0);
    // 获取列表并更新rows
    rows.value =
      tab.value === '正在运行'
