      ...
      <q-card-section class="q-px-none q-py-sm">
        <div class="row" style="height: 40px; line-height: 40px">
          ...
          <q-btn
            ...
            @click="toPage('/batch-compute', $event)"
          />
        </div>
      </q-card-section>
    </q-card>
...
async function getInstanceLength() {
  instanceLength.value = await InstanceService.getListNumberWithType(undefined, 2);
  runningInstanceLength.value = await InstanceService.getRunningNumberWithType(undefined, 2);
  finishedInstanceLength.value = await InstanceService.getFinishedNumberWithType(undefined, 2);
  warningInstanceLength.value = await InstanceService.getWarningNumberWithType(undefined, 2);
}
const loadData = () => {
  resetSelect();
  onRequest({
...
const onRequest = async (props: any) => {
  ...
  try {
    ...
    pagination.value.rowsNumber =
      tab.value === '正在运行'
        ? await InstanceService.getRunningNumberWithType(filter, 2)
        : tab.value === '最新完成'
        ? await InstanceService.getFinishedNumberWithType(filter, 2)
        : tab.value === '需要注意'
        ? await InstanceService.getWarningNumberWithType(filter, 2)
        : await InstanceService.getListNumberWithType(filter, 2);
    // 获取列表并更新rows
    rows.value =
      tab.value === '正在运行'
